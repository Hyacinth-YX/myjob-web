...
<div>
  ...
  <div class="row">
    <div class="col-4"> </div>
    <div class="col-5">
      <div class="text-h3" style="padding:50px 25px 5px 50px">
           holland test
      </div>
      <q-layout>
        <q-list>
          <test-item
            v-for="(question, index) in holland"
            :key="index"
            :question-data="question"
            :idx="index"
            :checklist.sync="checklist"
            :lastlist.sync="lastlist"
            class="q-ma-sm"
          ></test-item>
        </q-list>
      </q-layout>
  ...
  <div class="row" style="padding:50px 25px 50px 50px">
    <div class="col-5">
    </div>
    <div class="col-2">
   <q-btn label="提交结果" color="green" @click="sendtestresults"></q-btn>
    </div>
   <div class="col-6">
    </div>
  </div>
...
export default {
      ...
      sendtestresults:function() {
         ...
         for(let i=0; i < this.holland.length; i++){
          ...
          if(flag){
              ...
              for (let i = 0; i < 86; i++) {
               setTimeout(() => {
               document.documentElement.scrollTop =document.documentElement.scrollTop -2*(89-firstnum)
                }, 3 * i)
              }
  ...
  mounted(){
      ...
      this.summitcheck=[]
      for(let i=0; i < this.holland.length+1; i++){
          this.summitcheck[i] = false
      }
