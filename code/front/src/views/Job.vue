  ...
  <div>
    ...
    <div class="info-container row" style="min-height: 300px">
      ...
      <div class="info-block col-4 q-pa-md">
        <!--        标题-->
        <div class="text-h4 q-pa-sm text-bold">
          {{ information.jobSubName }}
        </div>
        <div class="text-h5 q-pa-sm text-bold text-blue">
        ...
        <div class="text-body1" >
          <div>
            <div>
              工作限制:
              {{ information.jobLimitExperience }}
              {{ information.jobLimitStudy }}
            </div>
            <div>
              工作地区:
              {{ information.jobArea }}
      ...
      <div class="info-block col-4 q-pa-md text-h5">
        ...
        <table
          ...
          style="width: 100%; font-family: SimSun, cursive"
        >
          <tr>
            <td>薪资均值</td>
            <td>
              <a class="text-primary">{{ jobData.salary.mean }}</a>
          ...
          <tr>
            <td>薪资最大值</td>
            <td>
              <a class="text-primary">{{ jobData.salary.max }}</a>
              <q-icon class="q-mb-sm" color="red" name="north" />
            </td>
          </tr>
          <tr>
            <td>薪资最小值</td>
            <td>
              <a class="text-primary">{{ jobData.salary.min }}</a>
              <q-icon class="q-mb-sm" color="red" name="north" />
            </td>
          </tr>
          <tr>
            <td>岗位数量</td>
            <td>
              <a class="text-primary">{{ jobData.number }}</a>
        ...
        <span class="row justify-center">
          <div class="col-6 text-caption" style="text-align: center">
            薪资中值/历史最大值
          </div>
          <div class="col-6 text-caption" style="text-align: center">
            当前岗位数量/历史平均
      ...
      <div class="q-pa-md col-4 info-block">
        <img :src="information.companyLogo" style="width:100%"/>
      </div>
    </div>
    ...
    <div class="row info-container">
      ...
      <div class="info-block col-6 q-pa-md">
    ...
    <div class="column info-container info-block">
      <div class="text-bold text-h4 q-pa-sm q-ma-md col">相关岗位推荐</div>
      <!--名称、要求、薪资、发布网站、时间、热门讨论-->
      <q-list class="col" separator>
        <q-item
          v-for="item of recommendJobs"
          :key="item.id"
...
<script>
import { WordCloud, Liquid, Gauge, Line } from "@antv/g2plot";
...
必须掌握计算机相关知识，熟练使用仿真工具MATLAB等，必须会一门编程语言。`,
  ...
  mounted: async function () {
    this.jobId = this.$route.query.jobId ?? this.jobId;
    let res = await this.$api.bigjob.getJobDetail(this.jobId);
    this.information = res.data.data;

    res = await this.$api.bigjob.getRecommend(this.jobId);
