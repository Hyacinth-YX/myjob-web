...
(function () {
  ...
  function init(frame) {
    ...
    var jumpPage = function (srcReferer, callbackUrl) {
      ...
      image.src =
        ...
        Math.round((new Date().getTime() - pageStartTime) / 1e3) +
        "&r=" +
        document.referrer;
    };
    var url = window.location.href;
    ...
    if (seed && ts && fileName) {
      ...
      seriesLoadScript("security-js/" + fileName + ".js", function () {
        ...
        try {
          code = new ABC().z(
            seed,
            parseInt(ts) + (480 + new Date().getTimezoneOffset()) * 60 * 1e3
          );
        } catch (e) {}
        if (code && callbackUrl) {
          Cookie.set("__zp_stoken__", code, expiredate, COOKIE_DOMAIN, "/");
          if (
            typeof window.wst != "undefined" &&
            typeof wst.postMessage == "function"
          ) {
            nativeParams = {
              name: "setWKCookie",
              params: {
  ...
  ready(function () {
    ...
    if (
      na.match(/micromessenger/i) == "micromessenger" ||
      na.match(/wkwebview/i) == "wkwebview"
    ) {
